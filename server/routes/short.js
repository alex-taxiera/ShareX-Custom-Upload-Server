...
async function post(req, res) {
    ...
    if (req.body.URL == undefined || req.body.URL == "" || req.body.URL == null) {
        res.redirect("/sharex/short?error=No URL Input");
        return res.end();
    }
    ...
    stream.once("open", fd => {
        ...
        let insecure = `/short?success=http://${req.headers.host}/sharex/${fileName}`
        let secure = `/short?success=https://${req.headers.host}/sharex/${fileName}`
